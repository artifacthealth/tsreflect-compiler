...
module ts {
    ...
    export function createProgram(rootNames: string[], options: CompilerOptions, host: InternalCompilerHost): Program {
        var program: Program;
        ...
        function processSourceFile(filename: string, isDefaultLib: boolean, refFile?: SourceFile, refPos?: number, refEnd?: number) {
            ...
            if (hasExtension(filename)) {
                ...
                else if (!(ret = findSourceFile(filename + ".ts", isDefaultLib, refFile, refPos, refEnd)) && !(ret = findSourceFile(filename + ".d.ts", isDefaultLib, refFile, refPos, refEnd))) {
                    diagnostic = Diagnostics.File_0_not_found;
                    filename += ".ts";
                }
            }
