...
module ts {
    ...
    export function createProgram(rootNames: string[], options: CompilerOptions, host: CompilerHost): Program {
        ...
        function processRootFile(filename: string, isDefaultLib: boolean) {
            var sourceFile = processSourceFile(normalizePath(filename), isDefaultLib);
            if(sourceFile && !isDefaultLib) {
                // flag files that was explicitly listed on the command-line
                sourceFile.flags |= NodeFlags.RootFile;
            }
        }

        function processSourceFile(filename: string, isDefaultLib: boolean, refFile?: SourceFile, refPos?: number, refEnd?: number): SourceFile {
