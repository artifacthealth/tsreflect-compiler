...
(function (ts) {
    (function (DeclarationFlag) {
        ...
        DeclarationFlag[DeclarationFlag["Exported"] = 0x00000001] = "Exported";
        DeclarationFlag[DeclarationFlag["Private"] = 0x00000002] = "Private";
        DeclarationFlag[DeclarationFlag["Public"] = 0x00000004] = "Public";
        DeclarationFlag[DeclarationFlag["Protected"] = 0x00000008] = "Protected";
        DeclarationFlag[DeclarationFlag["Ambient"] = 0x00000010] = "Ambient";
        DeclarationFlag[DeclarationFlag["Static"] = 0x00000020] = "Static";
        DeclarationFlag[DeclarationFlag["Optional"] = 0x00000040] = "Optional";
        DeclarationFlag[DeclarationFlag["RestParameter"] = 0x00000080] = "RestParameter";
        DeclarationFlag[DeclarationFlag["ExternalModule"] = 0x00000100] = "ExternalModule";
        DeclarationFlag[DeclarationFlag["HasNoDefaultLib"] = 0x00000200] = "HasNoDefaultLib";
        DeclarationFlag[DeclarationFlag["DeclarationFile"] = 0x00000400] = "DeclarationFile";
        DeclarationFlag[DeclarationFlag["RootFile"] = 0x00000800] = "RootFile";
    ...
    (function (DeclarationKind) {
        ...
        DeclarationKind[DeclarationKind["StringLiteralType"] = 26] = "StringLiteralType";
        DeclarationKind[DeclarationKind["TupleType"] = 27] = "TupleType";
...
(function (ts) {
    ...
    (function (NodeFlags) {
        ...
        NodeFlags[NodeFlags["RootFile"] = 0x00000800] = "RootFile";
    ...
    function createProgram(rootNames, options, host) {
        ...
        function processRootFile(filename, isDefaultLib) {
            ...
            if (sourceFile && !isDefaultLib) {
                // flag files that was explicitly listed on the command-line
                sourceFile.flags |= 2048 /* RootFile */;
            }
        }
...
(function (ts) {
    ...
    function lex() {
        var source = _source, length = source.length, value, begin, position, isSigned, charCode;
        while (_index < length) {
    ...
    function parseAnnotationValue(source) {
        _index = 0;
        _source = "" + source;
        var value = lex();
        if (peek() == '$') {
    ...
    ts.parseAnnotationValue = parseAnnotationValue;
})(ts || (ts = {}));
...
(function (ts) {
    function shouldEmitToOwnFile(sourceFile, compilerOptions) {
        if (ts.isDeclarationFile(sourceFile)) {
            return (sourceFile.flags & 2048 /* RootFile */) !== 0;
        }
    ...
    function getCommentText(currentSourceFile, node) {
    ...
    function emitDeclarations(program, resolver, diagnostics, jsFilePath, root) {
        ...
        function getJsDocAnnotations(jsDocComment) {
            ...
            for (var i = 0, l = jsDocComment.parseResults.tags.length; i < l; i++) {
                var tag = jsDocComment.parseResults.tags[i], name = tag.title;
                if (!ts.hasProperty(ignoreAnnotation, name)) {
                    ...
                    if (value === null || value === undefined) {
                        ...
                        try {
                            value = ts.parseAnnotationValue(value);
                        }
                        catch (e) {
                            diagnostics.push(ts.createDiagnosticForNode(jsDocComment.node, ts.CustomDiagnostics.Invalid_annotation_value_0, e.message));
                        }
                    }
        ...
        function emitDeclarationFlags(node) {
            if (node.flags & 128 /* Static */) {
                if (node.flags & 32 /* Private */) {
                    writer.writeFlags(2 /* Private */);
                }
                else if (node.flags & 64 /* Protected */) {
                    writer.writeFlags(8 /* Protected */);
                }
                writer.writeFlags(32 /* Static */);
            }
            else {
                if (node.flags & 32 /* Private */) {
                    writer.writeFlags(2 /* Private */);
        ...
        function emitModuleDeclaration(node) {
            if (resolver.isDeclarationVisible(node)) {
                ...
                if (isAmbientExternalModule(node)) {
                    ...
                    writer.writeFlags(256 /* ExternalModule */);
                }
                else {
        ...
        function emitTypeParameters(typeParameters) {
            function emitTypeParameter(node) {
                ...
                if (node.constraint) {
                    if (node.parent.kind !== 120 /* Method */ || !(node.parent.flags & 32 /* Private */)) {
        ...
        function emitPropertyDeclaration(node, description) {
            if (node.parent.kind == 174 /* ClassDeclaration */ || node.parent.kind == 121 /* Constructor */) {
        ...
        function emitAccessorDeclaration(node) {
            ...
            if (compilerOptions.accessors) {
                writer.writeBeginClassMember(node.kind === 122 /* GetAccessor */ ? 14 /* GetAccessor */ : 15 /* SetAccessor */);
                emitName(node);
                emitDeclarationFlags(node);
                emitJsDocComments(node);
        ...
        function emitSignatureDeclaration(node) {
            ...
            if (node.kind != 126 /* IndexSignature */ && node.kind != 123 /* SetAccessor */ && node.parameters.length > 0) {
            ...
            if (node.kind !== 121 /* Constructor */ && node.kind !== 123 /* SetAccessor */) {
                if (!(node.flags & 32 /* Private */)) {
        ...
        function emitParameterDeclaration(node) {
            ...
            if (node.flags & 8 /* Rest */) {
                writer.writeFlags(128 /* RestParameter */);
            }
        ...
        function isTypeAccessible(type, enclosingDeclaration) {
            if (type && !(type.flags & 16384 /* Anonymous */)) {
        ...
        function writeTypeToTextWriter(type, enclosingDeclaration, flags, writer) {
            ...
            function writeType(type, flags) {
                if (type.flags & ts.TypeFlags.Intrinsic) {
                    ...
                    writer.writeTypeReference(!(flags & 16 /* WriteOwnNameForAnyLike */) && (type.flags & 1 /* Any */) ? "any" : type.intrinsicName);
                }
            ...
            function writeTypeReference(type) {
                if (resolver.isArrayType(type) && !(flags & 1 /* WriteArrayAsGenericType */)) {
                    ...
                    for (var i = 0; i < type.typeArguments.length; i++) {
                        writeType(type.typeArguments[i], flags | 8 /* WriteArrowStyleSignature */);
                    }
                    writer.writeEnd();
                }
                writer.writeEnd();
            ...
            function writeTypeList(types) {
                writer.writeBeginType(27 /* TupleType */);
            ...
            function isOptionalProperty(propertySymbol) {
                ...
                return (propertySymbol.valueDeclaration.flags & 4 /* QuestionMark */) && propertySymbol.valueDeclaration.kind !== 118 /* Parameter */;
            }
        }
        ...
        function writeReferencePath(referencedFile) {
            var declFileName;
            if (referencedFile.flags & 1024 /* DeclarationFile */) {
                // declaration file
                declFileName = getOwnEmitOutputFilePath(referencedFile, program, ".d.json");
            }
            else {
                declFileName = shouldEmitToOwnFile(referencedFile, compilerOptions) ? getOwnEmitOutputFilePath(referencedFile, program, ".d.json") : ts.removeFileExtension(compilerOptions.out) + ".d.json"; // Global out file
            }
        ...
        if (root) {
            if (root.hasNoDefaultLib) {
                writer.writeFlags(512 /* HasNoDefaultLib */);
            }
            ...
            if (ts.forEach(program.getSourceFiles(), function (sourceFile) { return sourceFile.hasNoDefaultLib && !isExternalModuleOrDeclarationFile(sourceFile); })) {
                writer.writeFlags(512 /* HasNoDefaultLib */);
            }
...
(function (ts) {
    ts.CustomDiagnostics = {
        ...
        Invalid_annotation_value_0: { code: 10004, category: 1 /* Error */, key: "Invalid annotation value: {0}" },
...
(function (ts) {
    ...
    function compile(filenames, options) {
        ...
        var localOptions = loadConfig(ts.combinePaths(ts.normalizePath(process.cwd()), configFilename));
        if (localOptions) {
            mergeOptions(localOptions, options);
        }
        // look for config file in installed directory
        var globalOptions = loadConfig(ts.combinePaths(ts.normalizePath(__dirname), configFilename));
        if (globalOptions) {
            ...
            if (sys.fileExists(filename)) {
                try {
                    ...
                    try {
                        ...
                        var diagnostic = ts.createCompilerDiagnostic(ts.CustomDiagnostics.File_0_has_invalid_json_format_1, filename, e.message);
                    }
                }
                catch (e) {
                    var diagnostic = ts.createCompilerDiagnostic(ts.Diagnostics.Cannot_read_file_0_Colon_1, filename, e.message);
                }
        ...
        function mergeOptions(from, to) {
            for (var name in from) {
                if (ts.hasProperty(from, name)) {
                    if (!ts.hasProperty(to, name)) {
                        to[name] = from[name];
                    }
                }
            }
    ...
    function createCompilerHost(options) {
        ...
        return {
            getSourceFile: getSourceFile,
            getDefaultLibFilename: function () { return ts.combinePaths(ts.normalizePath(__dirname), options.libPath || "lib.d.ts"); },
            writeFile: writeFile,
