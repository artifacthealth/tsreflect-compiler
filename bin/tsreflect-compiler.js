    ...
    function createScanner(languageVersion, text, onError, onComment) {
        ...
        return {
            ...
            isReservedWord: function () {
                return token >= ts.SyntaxKind.FirstReservedWord && token <= ts.SyntaxKind.LastReservedWord;
            },
...
(function (ts) {
    ...
    (function (NodeFlags) {
        ...
        NodeFlags[NodeFlags["RootFile"] = 0x00000400] = "RootFile";
...
(function (ts) {
    ...
    function getJsDocComments(node, sourceFileOfNode) {
        return ts.filter(getLeadingCommentsOfNode(node, sourceFileOfNode), function (comment) {
            return isJsDocComment(comment);
        });
    ...
    function createSourceFile(filename, sourceText, languageVersion, version, isOpen) {
        ...
        function tryParse(callback) {
            return scanner.tryScan(function () {
                return lookAheadHelper(callback, false);
            });
        }
        ...
        function isListElement(kind, inErrorRecovery) {
            switch (kind) {
                ...
                case 8 /* BaseTypeReferences */:
                    return isIdentifier() && ((token !== 69 /* ExtendsKeyword */ && token !== 92 /* ImplementsKeyword */) || !lookAhead(function () {
                        return (nextToken(), isIdentifier());
                    }));
                case 9 /* VariableDeclarations */:
                case 14 /* TypeParameters */:
                    return isIdentifier();
        ...
        function isTypeMember() {
            switch (token) {
                ...
                case 9 /* OpenBracketToken */:
                    return true;
                default:
                    return isPropertyName() && lookAhead(function () {
                        return nextToken() === 7 /* OpenParenToken */ || token === 15 /* LessThanToken */ || token === 41 /* QuestionToken */ || token === 42 /* ColonToken */ || canParseSemicolon();
                    });
            }
        }
            ...
            switch (token) {
                ...
                case 78 /* NewKeyword */:
                    if (lookAhead(function () {
                        return nextToken() === 7 /* OpenParenToken */ || token === 15 /* LessThanToken */;
                    })) {
        ...
        function parseSwitchStatement() {
            ...
            var defaultClauses = ts.filter(node.clauses, function (clause) {
                return clause.kind === 158 /* DefaultClause */;
            });
        ...
        function isLabel() {
            return isIdentifier() && lookAhead(function () {
                return nextToken() === 42 /* ColonToken */;
            });
        }
        ...
        function isStatement(inErrorRecovery) {
            switch (token) {
                ...
                case 99 /* StaticKeyword */:
                    if (lookAhead(function () {
                        return nextToken() >= 55 /* Identifier */;
                    })) {
                        return false;
                    }
                default:
        ...
        function parseVariableDeclarationList(flags, noIn) {
            return parseDelimitedList(9 /* VariableDeclarations */, function () {
                return parseVariableDeclaration(flags, noIn);
            }, 0 /* Disallow */);
        }
        ...
        function isDeclaration() {
            switch (token) {
                ...
                case 106 /* ModuleKeyword */:
                    return lookAhead(function () {
                        return nextToken() >= 55 /* Identifier */ || token === 3 /* StringLiteral */;
                    });
                case 68 /* ExportKeyword */:
                    return lookAhead(function () {
                        return nextToken() === 43 /* EqualsToken */ || isDeclaration();
                    });
                case 104 /* DeclareKeyword */:
                case 98 /* PublicKeyword */:
                case 96 /* PrivateKeyword */:
        ...
        function getExternalModuleIndicator() {
            return ts.forEach(file.statements, function (node) {
                return node.flags & 1 /* Export */ || node.kind === 174 /* ImportDeclaration */ && node.externalModuleName || node.kind === 175 /* ExportAssignment */ ? node : undefined;
            });
        }
    ...
    function createProgram(rootNames, options, host) {
        ...
        program = {
            ...
            getCompilerHost: function () {
                return host;
            },
            getDiagnostics: getDiagnostics,
            getGlobalDiagnostics: getGlobalDiagnostics,
            getTypeChecker: function (fullTypeCheckMode) {
                return ts.createTypeChecker(program, fullTypeCheckMode);
            },
        ...
        function getDiagnostics(sourceFile) {
            return sourceFile ? ts.filter(errors, function (e) {
                return e.file === sourceFile;
            }) : errors;
        }

        function getGlobalDiagnostics() {
            return ts.filter(errors, function (e) {
                return !e.file;
            });
        }
        ...
        function processRootFile(filename, isDefaultLib) {
            var sourceFile = processSourceFile(ts.normalizePath(filename), isDefaultLib);
            if (sourceFile && !isDefaultLib) {
                sourceFile.flags |= 1024 /* RootFile */;
            }
        }
        ...
        function processSourceFile(filename, isDefaultLib, refFile, refPos, refEnd) {
            ...
            if (hasExtension(filename)) {
                ...
                } else if (!(ret = findSourceFile(filename, isDefaultLib, refFile, refPos, refEnd))) {
                    diagnostic = ts.Diagnostics.File_0_not_found;
                }
            } else {
                if (!((ret = findSourceFile(filename + ".ts", isDefaultLib, refFile, refPos, refEnd)) || (ret = findSourceFile(filename + ".d.ts", isDefaultLib, refFile, refPos, refEnd)))) {
                    diagnostic = ts.Diagnostics.File_0_not_found;
                    filename += ".ts";
                }
            }
        ...
        function verifyCompilerOptions() {
            ...
            var firstExternalModule = ts.forEach(files, function (f) {
                return isExternalModule(f) ? f : undefined;
            });
...
var ts;
(function (ts) {
    var TextFileWriter = (function () {
        function TextFileWriter(program, filePath) {
            this.filePath = filePath;
            this._compilerHost = program.getCompilerHost();
            this._compilerOptions = program.getCompilerOptions();
        }
        TextFileWriter.prototype.write = function (text, onError) {
            this._compilerHost.writeFile(this.filePath, text, this._compilerOptions.emitBOM, onError);
        };
        return TextFileWriter;
    })();
    ts.TextFileWriter = TextFileWriter;
})(ts || (ts = {}));
var ts;
(function (ts) {
    (function (DeclarationFlag) {
        DeclarationFlag[DeclarationFlag["None"] = 0] = "None";
        DeclarationFlag[DeclarationFlag["Exported"] = 1] = "Exported";
        DeclarationFlag[DeclarationFlag["Private"] = 1 << 1] = "Private";
        DeclarationFlag[DeclarationFlag["Public"] = 1 << 2] = "Public";
        DeclarationFlag[DeclarationFlag["Ambient"] = 1 << 3] = "Ambient";
        DeclarationFlag[DeclarationFlag["Static"] = 1 << 4] = "Static";
        DeclarationFlag[DeclarationFlag["Optional"] = 1 << 5] = "Optional";
        DeclarationFlag[DeclarationFlag["RestParameter"] = 1 << 6] = "RestParameter";
        DeclarationFlag[DeclarationFlag["ExternalModule"] = 1 << 7] = "ExternalModule";
    })(ts.DeclarationFlag || (ts.DeclarationFlag = {}));
    var DeclarationFlag = ts.DeclarationFlag;

    (function (DeclarationKind) {
        DeclarationKind[DeclarationKind["None"] = 0] = "None";

        DeclarationKind[DeclarationKind["Script"] = 1] = "Script";

        DeclarationKind[DeclarationKind["Interface"] = 2] = "Interface";
        DeclarationKind[DeclarationKind["Class"] = 3] = "Class";
        DeclarationKind[DeclarationKind["Enum"] = 4] = "Enum";
        DeclarationKind[DeclarationKind["Module"] = 5] = "Module";
        DeclarationKind[DeclarationKind["Container"] = 6] = "Container";
        DeclarationKind[DeclarationKind["Function"] = 7] = "Function";
        DeclarationKind[DeclarationKind["Variable"] = 8] = "Variable";
        DeclarationKind[DeclarationKind["Import"] = 9] = "Import";

        DeclarationKind[DeclarationKind["EnumMember"] = 10] = "EnumMember";

        DeclarationKind[DeclarationKind["Index"] = 11] = "Index";
        DeclarationKind[DeclarationKind["Field"] = 12] = "Field";
        DeclarationKind[DeclarationKind["Method"] = 13] = "Method";
        DeclarationKind[DeclarationKind["Constructor"] = 14] = "Constructor";
        DeclarationKind[DeclarationKind["Accessor"] = 15] = "Accessor";
        DeclarationKind[DeclarationKind["GetAccessor"] = 16] = "GetAccessor";
        DeclarationKind[DeclarationKind["SetAccessor"] = 17] = "SetAccessor";

        DeclarationKind[DeclarationKind["PropertySignature"] = 18] = "PropertySignature";
        DeclarationKind[DeclarationKind["ConstructSignature"] = 19] = "ConstructSignature";
        DeclarationKind[DeclarationKind["MethodSignature"] = 20] = "MethodSignature";
        DeclarationKind[DeclarationKind["IndexSignature"] = 21] = "IndexSignature";
        DeclarationKind[DeclarationKind["CallSignature"] = 22] = "CallSignature";

        DeclarationKind[DeclarationKind["FunctionType"] = 23] = "FunctionType";
        DeclarationKind[DeclarationKind["ArrayType"] = 24] = "ArrayType";
        DeclarationKind[DeclarationKind["ConstructorType"] = 25] = "ConstructorType";
        DeclarationKind[DeclarationKind["GenericType"] = 26] = "GenericType";
        DeclarationKind[DeclarationKind["ObjectType"] = 27] = "ObjectType";

        DeclarationKind[DeclarationKind["Extends"] = 28] = "Extends";
        DeclarationKind[DeclarationKind["Implements"] = 29] = "Implements";
        DeclarationKind[DeclarationKind["Constraint"] = 30] = "Constraint";

        DeclarationKind[DeclarationKind["ParameterList"] = 31] = "ParameterList";
        DeclarationKind[DeclarationKind["Parameter"] = 32] = "Parameter";

        DeclarationKind[DeclarationKind["TypeParameterList"] = 33] = "TypeParameterList";
        DeclarationKind[DeclarationKind["TypeParameter"] = 34] = "TypeParameter";

        DeclarationKind[DeclarationKind["TypeAnnotation"] = 35] = "TypeAnnotation";
        DeclarationKind[DeclarationKind["TypeArgumentList"] = 36] = "TypeArgumentList";
    })(ts.DeclarationKind || (ts.DeclarationKind = {}));
    var DeclarationKind = ts.DeclarationKind;

    var DeclarationWriter = (function () {
        function DeclarationWriter(writer) {
            this._currentState = {
                kind: 1 /* Script */,
                node: this._rootNode = {}
            };
        ...
        DeclarationWriter.prototype.writeBeginModuleElement = function (kind) {
            var node = {
        ...
        DeclarationWriter.prototype.writeBeginRemovableModuleElement = function (kind) {
            ...
            return {
                remove: function () {
                    var declarations = parentNode["declares"];
        ...
        DeclarationWriter.prototype.writeBeginClassMember = function (kind) {
            var node = {
                kind: kindToString(kind)
            };
        ...
        DeclarationWriter.prototype.writeBeginSignature = function (kind) {
            var node = {
                kind: kindToString(kind)
            };
        ...
        DeclarationWriter.prototype.writeBeginParameter = function () {
            ...
            switch (this._currentState.kind) {
                case 31 /* ParameterList */:
                    this._currentState.node.push(node);
                    break;
                case 21 /* IndexSignature */:
            ...
            this._pushState(32 /* Parameter */, node);
        };

        DeclarationWriter.prototype.writeBeginExtends = function () {
            switch (this._currentState.kind) {
                case 3 /* Class */:
                    this._pushState(28 /* Extends */);
                    break;
                case 2 /* Interface */:
                    this._pushArray(28 /* Extends */, "extends");
                    break;
                default:
        ...
        DeclarationWriter.prototype.writeBeginImplements = function () {
            this._pushArray(28 /* Extends */, "implements");
        };
        ...
        DeclarationWriter.prototype.writeEnd = function () {
            this._currentState = this._currentState.parent;

            ts.Debug.assert(this._currentState !== undefined, "Unmatched call to writeEnd");
        };

        DeclarationWriter.prototype.writeReference = function (path) {
            this._addToArray("references", path);
        };

        DeclarationWriter.prototype.writeName = function (name) {
        ...
        DeclarationWriter.prototype.writeFlags = function (flags) {
            if (this._currentState.kind == 9 /* Import */ && (flags & 1 /* Exported */) !== 0) {
        ...
        DeclarationWriter.prototype.writeValue = function (identifier) {
            this._setProperty("value", identifier);
        };

        DeclarationWriter.prototype.writeRequire = function (path) {
            this._setProperty("require", path);
        };

        DeclarationWriter.prototype.writeTypeReference = function (identifier) {
            this._setTypeOnCurrentNode(identifier);
        };

        DeclarationWriter.prototype.writeExportAssignment = function (identifier) {
            this._setProperty("export", identifier);
        };

        DeclarationWriter.prototype.close = function () {
            var _this = this;
            var diagnostics;

            this._writer.write(JSON.stringify(this._rootNode, null, "\t"), function (hostErrorMessage) {
                diagnostics = [];
                diagnostics.push(ts.createCompilerDiagnostic(ts.Diagnostics.Could_not_write_file_0_Colon_1, _this._writer.filePath, hostErrorMessage));
            });
        ...
        DeclarationWriter.prototype._setTypeOnCurrentNode = function (type) {
            switch (this._currentState.kind) {
                case 30 /* Constraint */:
                    ...
                    break;
                case 36 /* TypeArgumentList */:
                case 33 /* TypeParameterList */:
                case 29 /* Implements */:
        ...
        DeclarationWriter.prototype._isReturnType = function () {
            switch (this._currentState.kind) {
                case 13 /* Method */:
                case 20 /* MethodSignature */:
                case 14 /* Constructor */:
                case 19 /* ConstructSignature */:
                case 25 /* ConstructorType */:
                case 16 /* GetAccessor */:
                case 7 /* Function */:
                case 23 /* FunctionType */:
                case 22 /* CallSignature */:
                    return true;
            }
        ...
        DeclarationWriter.prototype._pushArray = function (kind, name) {
            this._pushState(kind, this._ensureArray(name));
        };

        DeclarationWriter.prototype._addToArray = function (name, value) {
            this._ensureArray(name).push(value);
        };

        DeclarationWriter.prototype._ensureArray = function (name) {
            return this._ensureProperty(name, []);
        };

        DeclarationWriter.prototype._ensureProperty = function (name, value) {
        ...
        DeclarationWriter.prototype._setProperty = function (name, value) {
            this._currentState.node[name] = value;
        };

        DeclarationWriter.prototype._pushState = function (kind, node) {
            this._currentState = {
    ...
    ts.DeclarationWriter = DeclarationWriter;
    ...
    kindToStringMap[2 /* Interface */] = "interface";
    kindToStringMap[3 /* Class */] = "class";
    kindToStringMap[4 /* Enum */] = "enum";
    kindToStringMap[5 /* Module */] = "module";
    kindToStringMap[6 /* Container */] = "container";
    kindToStringMap[7 /* Function */] = "function";
    kindToStringMap[8 /* Variable */] = "variable";
    kindToStringMap[9 /* Import */] = "import";
    kindToStringMap[11 /* Index */] = "index";
    kindToStringMap[12 /* Field */] = "field";
    kindToStringMap[13 /* Method */] = "method";
    kindToStringMap[14 /* Constructor */] = "constructor";
    kindToStringMap[16 /* GetAccessor */] = "accessor";
    kindToStringMap[17 /* SetAccessor */] = "accessor";
    kindToStringMap[18 /* PropertySignature */] = "property";
    kindToStringMap[19 /* ConstructSignature */] = "constructor";
    kindToStringMap[20 /* MethodSignature */] = "method";
    kindToStringMap[21 /* IndexSignature */] = "index";
    kindToStringMap[22 /* CallSignature */] = "call";
    kindToStringMap[23 /* FunctionType */] = "function";
    kindToStringMap[24 /* ArrayType */] = "array";
    kindToStringMap[25 /* ConstructorType */] = "constructor";
    kindToStringMap[26 /* GenericType */] = "generic";
    kindToStringMap[27 /* ObjectType */] = "object";
})(ts || (ts = {}));
    ...
    function emitFiles(resolver) {
        ...
        function shouldEmitToOwnFile(sourceFile) {
            if (sourceFile.flags & 512 /* DeclarationFile */) {
                return (sourceFile.flags & 1024 /* RootFile */) !== 0;
            }

            return (ts.isExternalModule(sourceFile) || !compilerOptions.out) && !ts.fileExtensionIs(sourceFile.filename, ".js");
        }
        ...
        function emitDeclarations(writer, jsFilePath, root) {
            ...
            function makeAliasesVisible(importDeclarations) {
            ...
            function hasJsDocComment(declaration) {
                if (declaration) {
            ...
            function getJsDocComment(declaration) {
                if (declaration) {
                    var jsDocComments = ts.getJsDocComments(declaration, currentSourceFile);
                    if (jsDocComments && jsDocComments.length > 0) {
            ...
            function emitDeclarationFlags(node) {
                if (node.flags & 64 /* Static */) {
                    if (node.flags & 32 /* Private */) {
                        writer.writeFlags(ts.DeclarationFlag.Private);
                    }
                    writer.writeFlags(ts.DeclarationFlag.Static);
                } else {
                    if (node.flags & 32 /* Private */) {
                        writer.writeFlags(ts.DeclarationFlag.Private);
                    } else if (node.parent === currentSourceFile) {
                        if (node.flags & 1 /* Export */) {
                            writer.writeFlags(1 /* Exported */);
                        }

                        if (node.kind !== 170 /* InterfaceDeclaration */) {
                            writer.writeFlags(ts.DeclarationFlag.Ambient);
                        }
                    }
                }
            }
            ...
            function writeImportDeclaration(node) {
                var handle = writer.writeBeginRemovableModuleElement(9 /* Import */);
            ...
            function isAmbientExternalModule(node) {
                if (node.flags & 2 /* Ambient */) {
            ...
            function emitEnumDeclaration(node) {
                if (resolver.isDeclarationVisible(node)) {
                    writer.writeBeginModuleElement(4 /* Enum */);
            ...
            function emitHeritageClause(typeReferences, isImplementsList) {
                ...
                function emitTypeOfTypeReference(node) {
                    ...
                    writeTypeAtLocation(node, enclosingDeclaration, 1 /* WriteArrayAsGenericType */ | 2 /* UseTypeOfFunction */, writer);
            ...
            function emitClassDeclaration(node) {
                function emitParameterProperties(constructorDeclaration) {
                    if (constructorDeclaration) {
                        var jsDocComment = getJsDocComment(constructorDeclaration), paramDescriptions;
                ...
                if (resolver.isDeclarationVisible(node)) {
                    writer.writeBeginModuleElement(3 /* Class */);
            ...
            function emitInterfaceDeclaration(node) {
                if (resolver.isDeclarationVisible(node)) {
                    writer.writeBeginModuleElement(2 /* Interface */);
            ...
            function emitPropertyDeclaration(node, description) {
                if (node.parent.kind == 169 /* ClassDeclaration */ || node.parent.kind == 117 /* Constructor */) {
                    writer.writeBeginClassMember(12 /* Field */);
                } else {
                    writer.writeBeginSignature(18 /* PropertySignature */);
                }
            ...
            function emitVariableStatement(node) {
                var hasDeclarationWithEmit = ts.forEach(node.declarations, function (varDeclaration) {
                ...
                if (hasDeclarationWithEmit) {
                    ...
                    for (var i = 0, n = declarations.length; i < n; i++) {
                        ...
                        writer.writeBeginModuleElement(8 /* Variable */);
            ...
            function emitFunctionDeclaration(node) {
                if ((node.kind !== 167 /* FunctionDeclaration */ || resolver.isDeclarationVisible(node)) && !resolver.isImplementationOfOverload(node)) {
                    if (node.kind === 167 /* FunctionDeclaration */) {
                        writer.writeBeginModuleElement(7 /* Function */);
                        emitName(node);
                    } else if (node.kind === 117 /* Constructor */) {
                        writer.writeBeginClassMember(14 /* Constructor */);
                    } else {
                        if (node.parent.kind == 169 /* ClassDeclaration */) {
                            writer.writeBeginClassMember(13 /* Method */);
                        } else {
                            writer.writeBeginSignature(20 /* MethodSignature */);
                        }
            ...
            function emitConstructSignatureDeclaration(node) {
                writer.writeBeginSignature(19 /* ConstructSignature */);
                emitJsDocComments(node);
                emitSignatureDeclaration(node);
                writer.writeEnd();
            }
            ...
            function emitCallSignatureDeclaration(node) {
                writer.writeBeginSignature(22 /* CallSignature */);
                emitJsDocComments(node);
                emitSignatureDeclaration(node);
                writer.writeEnd();
            }
            ...
            function emitIndexSignatureDeclaration(node) {
                writer.writeBeginSignature(21 /* IndexSignature */);
                emitJsDocComments(node);
                emitSignatureDeclaration(node);
                writer.writeEnd();
            }
            ...
            function emitParameterDeclaration(node) {
                ...
                if (node.flags & 8 /* Rest */) {
                    writer.writeFlags(ts.DeclarationFlag.RestParameter);
                }
            ...
            function writeTypeToTextWriter(type, enclosingDeclaration, flags, writer) {
                ...
                function writeType(type, allowFunctionOrConstructorTypeLiteral) {
                    if (type.flags & ts.TypeFlags.Intrinsic) {
                        writer.writeTypeReference(type.intrinsicName);
                    } else if (type.flags & 4096 /* Reference */) {
                        writeTypeReference(type);
                    } else if (type.flags & (1024 /* Class */ | 2048 /* Interface */ | 128 /* Enum */ | 512 /* TypeParameter */)) {
                        writeSymbol(type.symbol, enclosingDeclaration, ts.SymbolFlags.Type);
                        ...
                        writer.writeTypeReference(type.text);
                    } else {
                ...
                function writeTypeReference(type) {
                    if (resolver.isArrayType(type) && !(flags & 1 /* WriteArrayAsGenericType */)) {
                        writer.writeBeginType(24 /* ArrayType */);
                        writeType(type.typeArguments[0], false);
                    } else {
                        writer.writeBeginType(26 /* GenericType */);
                ...
                function writeAnonymousType(type, allowFunctionOrConstructorTypeLiteral) {
                    ...
                    function shouldWriteTypeOfFunctionSymbol() {
                        if (type.symbol) {
                            var isStaticMethodSymbol = !!(type.symbol.flags & 2048 /* Method */ && ts.forEach(type.symbol.declarations, function (declaration) {
                                return declaration.flags & 64 /* Static */;
                            }));
                            var isNonLocalFunctionSymbol = !!(type.symbol.flags & 8 /* Function */) && (type.symbol.parent || ts.forEach(type.symbol.declarations, function (declaration) {
                                return declaration.parent.kind === 177 /* SourceFile */ || declaration.parent.kind === 173 /* ModuleBlock */;
                            }));
                ...
                function writeLiteralType(type, allowFunctionOrConstructorTypeLiteral) {
                    ...
                    if (!resolved.properties.length && !resolved.stringIndexType && !resolved.numberIndexType) {
                        if (!resolved.callSignatures.length && !resolved.constructSignatures.length) {
                            writer.writeBeginType(27 /* ObjectType */);
                            writer.writeEnd();
                            return;
                        }
                        ...
                        if (allowFunctionOrConstructorTypeLiteral) {
                            if (resolved.callSignatures.length === 1 && !resolved.constructSignatures.length) {
                                writer.writeBeginType(23 /* FunctionType */);
                                emitSignatureDeclaration(resolved.callSignatures[0].declaration);
                                writer.writeEnd();
                                return;
                            }
                            if (resolved.constructSignatures.length === 1 && !resolved.callSignatures.length) {
                                writer.writeBeginType(25 /* ConstructorType */);
            ...
            function emitNode(node) {
                switch (node.kind) {
                    ...
                    case 120 /* CallSignature */:
                        return emitCallSignatureDeclaration(node);
                    case 122 /* IndexSignature */:
                        return emitIndexSignatureDeclaration(node);
                    case 118 /* GetAccessor */:
                    case 119 /* SetAccessor */:
            ...
            function writeReferencePath(referencedFile) {
                ...
                if (referencedFile.flags & 512 /* DeclarationFile */) {
                    declFileName = ts.getModuleNameFromFilename(referencedFile.filename) + ".d.json";
                } else {
                    declFileName = shouldEmitToOwnFile(referencedFile) ? getOwnEmitOutputFilePath(referencedFile, ".d.json") : ts.getModuleNameFromFilename(compilerOptions.out) + ".d.json";
                }
            ...
            ts.forEach(aliasDeclarationsToRemove, function (aliasInfo) {
    ...
    function createTypeChecker(program, fullTypeCheck) {
        ...
        var checker = {
            ...
            getNodeCount: function () {
                return ts.sum(program.getSourceFiles(), "nodeCount");
            },
            getIdentifierCount: function () {
                return ts.sum(program.getSourceFiles(), "identifierCount");
            },
            getSymbolCount: function () {
                return ts.sum(program.getSourceFiles(), "symbolCount");
            },
            ...
            if (!symbolLinks.exportAssignSymbol) {
                ...
                if (exportInformation.exportAssignments.length) {
                    if (exportInformation.exportAssignments.length > 1) {
                        ts.forEach(exportInformation.exportAssignments, function (node) {
                            return error(node, ts.Diagnostics.A_module_cannot_have_more_than_one_export_assignment);
                        });
                    }
            ...
            function getAccessibleSymbolChainFromSymbolTable(symbols) {
                ...
                function isAccessible(symbolFromSymbolTable, resolvedAliasSymbol) {
                    if (symbol === (resolvedAliasSymbol || symbolFromSymbolTable)) {
                        return !ts.forEach(symbolFromSymbolTable.declarations, function (declaration) {
                            return hasExternalModuleSymbol(declaration);
        ...
        function isSymbolAccessible(symbol, enclosingDeclaration, meaning) {
            if (symbol && enclosingDeclaration && !(symbol.flags & 262144 /* TypeParameter */)) {
                ...
                var symbolExternalModule = ts.forEach(initialSymbol.declarations, function (declaration) {
                    return getExternalModuleContainer(declaration);
                });
        ...
        function symbolToString(symbol, enclosingDeclaration, meaning) {
            ...
            if (enclosingDeclaration && !(symbol.flags & 262144 /* TypeParameter */)) {
                ...
                while (symbol) {
                    ...
                    if (accessibleSymbolChain) {
                        currentSymbolName = ts.map(accessibleSymbolChain, function (accessibleSymbol) {
                            return getSymbolName(accessibleSymbol);
                        }).join(".");
                    } else {
                        if (!isFirstName && ts.forEach(symbol.declarations, function (declaration) {
                            return hasExternalModuleSymbol(declaration);
                        })) {
                            break;
                        }
                    ...
                    if ((symbol.flags & 262144 /* TypeParameter */) && isTypeParameterReferenceIllegalInConstraint(node, symbol)) {
                        ...
                        if (type.flags & (1024 /* Class */ | 2048 /* Interface */) && type.flags & 4096 /* Reference */) {
                            ...
                            if (node.typeArguments && node.typeArguments.length === typeParameters.length) {
                                type = createTypeReference(type, ts.map(node.typeArguments, function (t) {
        ...
        function isContextSensitiveExpression(node) {
            switch (node.kind) {
                case 136 /* FunctionExpression */:
                case 137 /* ArrowFunction */:
                    return !node.typeParameters && !ts.forEach(node.parameters, function (p) {
                        return p.type;
                    });
                case 128 /* ObjectLiteral */:
                    return ts.forEach(node.properties, function (p) {
                        return p.kind === 129 /* PropertyAssignment */ && isContextSensitiveExpression(p.initializer);
                    });
                case 127 /* ArrayLiteral */:
                    return ts.forEach(node.elements, function (e) {
                        return isContextSensitiveExpression(e);
                    });
                case 141 /* ConditionalExpression */:
        ...
        function isPropertyIdenticalTo(sourceProp, targetProp) {
            return isPropertyIdenticalToRecursive(sourceProp, targetProp, false, function (s, t, _reportErrors) {
                ...
                if (n.kind === 55 /* Identifier */) {
                    ...
                    if (referencedSymbol && referencedSymbol !== unknownSymbol && getSymbol(parameterDeclaration.parent.locals, referencedSymbol.name, ts.SymbolFlags.Value) === referencedSymbol) {
                        if (referencedSymbol.valueDeclaration.kind === 114 /* Parameter */) {
                            ...
                            var enclosingOrReferencedParameter = ts.forEach(parameterDeclaration.parent.parameters, function (p) {
                                return p === parameterDeclaration || p === referencedSymbol.valueDeclaration ? p : undefined;
                            });
            ...
            if (node.parent.baseType) {
                if (containsSuperCall(node.body)) {
                    var superCallShouldBeFirst = ts.forEach(node.parent.members, isInstancePropertyWithInitializer) || ts.forEach(node.parameters, function (p) {
                        return p.flags & (16 /* Public */ | 32 /* Private */);
                    });
        ...
        function checkCollisionWithIndexVariableInGeneratedCode(node, name) {
            ...
            var current = node;
            while (current) {
                var definedOnCurrentLevel = ts.forEach(symbol.declarations, function (d) {
                    return d.parent === current ? d : undefined;
                });
        ...
        function checkIndexConstraints(type) {
            function checkIndexConstraintForProperty(prop, propertyType, indexDeclaration, indexType, indexKind) {
                ...
                if (prop.parent === type.symbol) {
                    ...
                    var someBaseClassHasBothPropertyAndIndexer = ts.forEach(type.baseTypes, function (base) {
                        return getPropertyOfType(base, prop.name) && getIndexTypeOfType(base, indexKind);
                    });
            ...
            if (stringIndexType && numberIndexType) {
                ...
                if (!errorNode && (type.flags & 2048 /* Interface */)) {
                    var someBaseTypeHasBothIndexers = ts.forEach(type.baseTypes, function (base) {
                        return getIndexTypeOfType(base, 0 /* String */) && getIndexTypeOfType(base, 1 /* Number */);
                    });
        ...
        function getDiagnostics(sourceFile) {
            if (sourceFile) {
                ...
                return ts.filter(getSortedDiagnostics(), function (d) {
        ...
        function isValidImportDeclaration(node) {
            var symbol = getSymbolOfNode(node);
            return resolveImport(symbol) != unknownSymbol;
        }
        ...
        function invokeEmitter() {
            var resolver = {
                ...
                isValidImportDeclaration: isValidImportDeclaration,
                isArrayType: isArrayType,
                getSignatureFromDeclaration: getSignatureFromDeclaration,
                getSymbolOfNode: getSymbolOfNode,
                getTypeFromTypeNode: getTypeFromTypeNode,
                resolveObjectTypeMembers: resolveObjectTypeMembers,
                symbolToString: symbolToString,
                getTypeOfSymbol: getTypeOfSymbol,
                getReturnTypeOfSignature: getReturnTypeOfSignature,
                getPropertiesOfType: getPropertiesOfType,
        ...
        function createDeclarationWriter(outputFilePath) {
            return new ts.DeclarationWriter(new ts.TextFileWriter(program, outputFilePath));
        }
...
(function (ts) {
    ...
    function printHelp() {
        ...
        syntax += "tsreflect [" + getDiagnosticText(ts.Diagnostics.options) + "] [" + getDiagnosticText(ts.Diagnostics.file) + " ...]";

        output += getDiagnosticText(ts.Diagnostics.Syntax_Colon_0, syntax);
        output += sys.newLine + sys.newLine;

        var padding = makePadding(marginLength);
        output += getDiagnosticText(ts.Diagnostics.Examples_Colon_0, makePadding(marginLength - examplesLength) + "tsreflect hello.ts") + sys.newLine;
...
//# sourceMappingURL=tsreflect-compiler.js.map
